...
@theme {
  ...
  --color-brand-dark: #13151A;
  --color-brand-card: #F3F3F3;
  --color-brand-muted: #EAEAEA;
}
...
/* Ensure Lexend applies strictly to all elements */
...
/* Custom scrollbar matching monochrome aesthetic */
::-webkit-scrollbar {
  width: 8px;
  height: 8px;
}
    ...
    <div className="min-h-screen bg-white text-[#13151A] flex flex-col font-sans selection:bg-[#13151A] selection:text-white">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-30 bg-white/95 backdrop-blur-md border-t border-neutral-200 p-2.5 px-4 flex items-center space-x-3 shadow-lg">
        <button
          onClick={() => {
            if (currentPage === 'home') {
              const heroForm = document.getElementById('hero-phone-input');
              if (heroForm) {
                heroForm.focus();
